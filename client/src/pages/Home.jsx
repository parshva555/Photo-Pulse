      ...
      <div className="w-full flex gap-2 lg:gap-4 pt-5 pb-10 h-full">
        ...
        <div className="flex-1 h-full px-4 flex flex-col gap-6 overflow-y-auto">
          ...
          className='bg-primary px-4 rounded-lg'>
            ...
            {errMsg?.message && (
              <span role='alert'
              className={`text-sm ${errMsg?.status === 'failed' ? "text-[#f64949fe]" : 'text-[#2ba250fe]'} mt-0.5`}></span>
            )}
            <div className='flex items-center justify-between py-4'>
              ...
              <label htmlFor="vgiUpload" className='flex items-center gap-1 text-base text-ascent-2 hover:text-ascent-1 cursor-pointer' >
                <input type="file" onChange={(e) => setFile(e.target.files[0])} 
                className='hidden'
                id="vgiUpload"
                data-max-size='5120'
              ...
              {posting ? (
                ...
                type='submit'
                title='Post'
                containerStyles='bg-[#e09200] text-white py-1 px-6 rounded-full font-semibold text-sm'/>
              )}
              </div>
            </div>
          </form>
